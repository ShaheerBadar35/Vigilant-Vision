...
# Load Keras violence model
custom_objects = {'DepthwiseConv2D': CustomDepthwiseConv2D}
VIOLENCE_MODEL = load_model('Sussy-Model/modelnew.h5', custom_objects=custom_objects)
VIOLENCE_CLASSES = ["NonViolence", "Violence"]
VIOLENCE_THRESHOLD = 0.6
MIN_CONSECUTIVE = 3
MIN_CONSECUTIVE_LIVE = 8

# Load PyTorch action classifier
...
class LRCNWithAttention(torch.nn.Module):
    def __init__(self, num_classes=3, sequence_length=8):
        super().__init__()
        self.cnn = torch.nn.Sequential(
            torch.nn.Conv2d(3, 32, 3, padding=1), torch.nn.BatchNorm2d(32), torch.nn.ReLU(),
            torch.nn.MaxPool2d(4), torch.nn.Dropout(0.4),
            torch.nn.Conv2d(32, 64, 3, padding=1), torch.nn.BatchNorm2d(64), torch.nn.ReLU(),
            torch.nn.MaxPool2d(4), torch.nn.Dropout(0.4),
        )
        self.flatten_dim = 64 * (64 // 16) * (64 // 16)
        self.lstm = torch.nn.LSTM(self.flatten_dim, 64, batch_first=True, bidirectional=False)
...
ACTION_CLASSES = ["explosion", "fighting", "shooting-robbery"]
IDX_TO_ACTION = {i: c for i, c in enumerate(ACTION_CLASSES)}
SEQUENCE_LENGTH = 8
IMAGE_SIZE = 64

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
ACTION_MODEL = LRCNWithAttention().to(device)
state_dict = torch.load('Sussy-Model/3class_model.pth', map_location=device)
...
def add_alert_to_firestore(camera_id, location_name, alert_type, detected_value, timestamp, action, status="pending"):
    print("Adding alert to Firestore")
...
def log_alert(location_name, alert_type, action, timestamp, camera_id=None, detected_value=None, image=None, status="pending"):
    ...
    # storing to firebase
    add_alert_to_firestore(camera_id,location_name,alert_type,detected_value,timestamp,action,status)
...
#----- Thread-local storage for per-thread alert cooldown
thread_data = local()
...
def CC_process_video_alternative(video_path, model, output_path, conf_threshold=0.25, frame_skip=10, detection_threshold=20, cooldown_seconds=90, count_change_threshold=3,location_name = "MainHall"):
    ...
    print("Crowd Count UPLOAD VIDEO CALLED")
        ...
        if frame_people_detected >= detection_threshold:
            ...
            if time_since_last_alert > cooldown_seconds:
                if count_difference >=count_change_threshold and frame_people_detected > last_crowd_count:
                    ...
                    log_alert(location_name, "Crowd", action, timestamp, "Uploaded Video", frame_people_detected)
...
def CC_process_webcam_feed(frame, model, conf_threshold=0.25, frame_skip=10, detection_threshold=2, cooldown_seconds=90, count_change_threshold=0,location_name = "Webcam Location"):
    ...
    if not hasattr(thread_data, "frame_count"):
        thread_data.frame_count = 0

    if not hasattr(thread_data, "last_crowd_alert_time"):
        thread_data.last_crowd_alert_time = 0

    if not hasattr(thread_data, "last_crowd_count"):
        thread_data.last_crowd_count = 0

    if not hasattr(thread_data, "last_threshold_check_time"):
        thread_data.last_threshold_check_time = 0
    
    # skiping frames to reduce computation
    if thread_data.frame_count % frame_skip != 0:
        thread_data.frame_count += 1
        return "crowd", 0  
    ...
    time_since_last_alert = current_time - thread_data.last_crowd_alert_time
    print("Last count:", thread_data.last_crowd_count)
    print("Current count:", total_people_detected_in_frame)
    count_difference = abs(total_people_detected_in_frame - thread_data.last_crowd_count)
    print("Count difference: ",count_difference)
    time_since_last_threshold = current_time - thread_data.last_threshold_check_time
    print(count_change_threshold)
    ...
    if total_people_detected_in_frame >= detection_threshold:
        ...
        if time_since_last_alert > cooldown_seconds:
            ...
            if count_difference >=count_change_threshold and total_people_detected_in_frame > thread_data.last_crowd_count:
                print('-------inside2')
                thread_data.last_crowd_alert_time = time.time()
                thread_data.last_crowd_count = total_people_detected_in_frame
        ...
        if time_since_last_threshold > 180:
            thread_data.last_crowd_count = 0
            thread_data.last_threshold_check_time = time.time()

    return "crowd", total_people_detected_in_frame


#MASK MODEL FUNCTIONS
def MASK_detect_objects_from_webcam(frame, model, cooldown_seconds = 0,location_name = "Webcam Location",mask_mode="No-Mask"):
        ...
        for box, class_id, track_id in zip(boxes, class_ids, track_ids):
            ...
            if label.lower() in ["without_mask", "mask_weared_incorrect"]:
                if track_id not in recent_detections_cache['mask']:
                    ...
                    print('-----',mask_mode)
                    print('-----',cooldown_seconds)
                    print('------',current_time)
                    

                    if not hasattr(thread_data, "last_mask_alert_time"):
                        thread_data.last_mask_alert_time = 0
                    
                    print('------done')
                    if (current_time - thread_data.last_mask_alert_time) > cooldown_seconds:
                        print("----inside logging alert----")
                        thread_data.last_mask_alert_time = time.time()
                        timestamp = datetime.now()
                        action = "Take action against attendees with no mask"
                        log_alert(location_name, mask_mode, action, timestamp, "Webcam", no_mask_detections)

    return no_mask_detections


def MASK_process_video_for_detections(video_path, model, location_name, mask_mode, cooldown_seconds=0):
            ...
            for box, class_id, track_id in zip(boxes, class_ids, track_ids):
                ...
                if label.lower() in ["without_mask", "mask_weared_incorrect"] and track_id not in recent_detections:
                    ...
                    if (current_time - last_alert_time) > cooldown_seconds:
                        ...
                        timestamp = datetime.now()
                        action = f"Take action against attendees with {mask_mode}"
                          # Replace with dynamic location
                        log_alert(location_name, mask_mode, action, timestamp, "Uploaded Video", 1)
        ...
        for box, class_id, track_id in zip(boxes, class_ids, track_ids):
            ...
            if label.lower() == "no-queue":
                if track_id not in recent_detections_cache['queue']:
                    ...
                    if not hasattr(thread_data, "last_queue_alert_time"):
                        thread_data.last_queue_alert_time = 0

                    if (current_time - thread_data.last_queue_alert_time) > cooldown_seconds:

                        thread_data.last_queue_alert_time = time.time()
                        timestamp = datetime.now()
            ...
            for box, class_id, track_id in zip(boxes, class_ids, track_ids):
                ...
                if label.lower() == "no-queue" and track_id not in recent_detections:
                    ...
                    if (current_time - last_alert_time) > cooldown_seconds:
                        ...
                        timestamp = datetime.now()
                        action = "Ensure attendees form queue"
                        log_alert(location_name, "No-Queue", action, timestamp, "Uploaded Video", 1)
        ...
        for box, class_id, track_id in zip(boxes, class_ids, track_ids):
            ...
            if label.lower() in ["cigarette", "smoke"]:
                if track_id not in recent_detections_cache['smoke']:
                    ...
                    if not hasattr(thread_data, "last_smoke_alert_time"):
                        thread_data.last_smoke_alert_time = 0

                    if (current_time - thread_data.last_smoke_alert_time) > cooldown_seconds:

                        thread_data.last_smoke_alert_time = time.time()
                        timestamp = datetime.now()
...
def SMOKE_process_video_for_detections(video_path,model, cooldown_seconds = 0,location_name = "MainHall"):
    """Process a video for mask detections and save snapshots to the database."""
    print("SMOKE UPLOAD VIDEO CALLED")
    print("cooldown_seconds: ",cooldown_seconds)
    cap = cv2.VideoCapture(video_path)
    last_alert_time = 0
    count = 0
            ...
            for box, class_id, track_id in zip(boxes, class_ids, track_ids):
                ...
                if label.lower() in ["cigarette", "smoke"] and track_id not in recent_detections:
                    ...
                    if (current_time - last_alert_time) > cooldown_seconds:
                        ...
                        timestamp = datetime.now()
                        action = "Take action against attendees smoking"
                        log_alert(location_name, "Smoke", action, timestamp, "Uploaded Video", 1)
...
def preprocess_frame_keras(frame):
    rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    resized_frame = cv2.resize(rgb_frame, (128, 128))
    normalized_frame = resized_frame / 255.0
    final_frame = np.expand_dims(normalized_frame, axis=0)
    return final_frame

def predict_action_sequence(action_model, frames, transform, device):
    # frames: list of numpy arrays (BGR)
    imgs = [transform(Image.fromarray(cv2.cvtColor(f, cv2.COLOR_BGR2RGB)).convert('RGB')) for f in frames]
    imgs = torch.stack(imgs).unsqueeze(0).to(device)
    with torch.no_grad():
        out = action_model(imgs)
        pred = out.argmax(dim=1).item()
    return pred

def has_consecutive_above(arr, threshold, min_run):
    run = 0
    indices = []
    best_indices = []
    for i, v in enumerate(arr):
        if v > threshold:
            ...
            indices.append(i)
            if run >= min_run:
                best_indices = indices[-run:]
        else:
            run = 0
            indices = []
    return (len(best_indices) >= min_run), best_indices


def pipeline_process_video(input_path, output_path, location_name="MainHall", cooldown_seconds=0):
    ...
    while cap.isOpened():
        ...
        processed = preprocess_frame_keras(frame)
        pred = VIOLENCE_MODEL.predict(processed, verbose=0)
        violence_prob = float(pred[0])
        violence_probs.append(violence_prob)

        label = "Violence" if violence_prob > VIOLENCE_THRESHOLD else "NonViolence"
        color = (0, 0, 255) if label == "Violence" else (0, 255, 0)
        cv2.putText(frame, f"{label} ({violence_prob:.2f})", (10, 30),
    ...
    is_violent, best_indices = has_consecutive_above(violence_probs, VIOLENCE_THRESHOLD, MIN_CONSECUTIVE)
    if is_violent:
        video_label = "Violence"
        anomaly_frames = [all_frames[i] for i in best_indices]
        if len(anomaly_frames) < SEQUENCE_LENGTH:
            pad_count = SEQUENCE_LENGTH - len(anomaly_frames)
            seq = anomaly_frames + [anomaly_frames[-1]] * pad_count
            pred = predict_action_sequence(ACTION_MODEL, seq, val_transform, device)
            action_label = IDX_TO_ACTION[pred]
        else:
            preds = []
            for i in range(0, len(anomaly_frames) - SEQUENCE_LENGTH + 1, 1):
                seq = anomaly_frames[i:i+SEQUENCE_LENGTH]
                pred = predict_action_sequence(ACTION_MODEL, seq, val_transform, device)
                preds.append(pred)
            if preds:
                most_common = Counter(preds).most_common(1)[0][0]
                action_label = IDX_TO_ACTION[most_common]
            else:
                action_label = "anomaly (unknown action)"
        # Log to DB and alert
        _, image_buffer = cv2.imencode('.jpg', anomaly_frames[0])
        image_data = image_buffer.tobytes()
        log_detection_to_db(os.path.basename(input_path), "anamoly", 1, image_data, action_label)
        timestamp = datetime.now()
        action = f"{action_label} Detected!! Take action"
        log_alert(location_name, "Suspicious", action, timestamp, os.path.basename(input_path), 1)
    else:
        video_label = "NonViolence"
        action_label = "normal"

    return f"{video_label} | {action_label}"

from collections import deque

# Global buffer for each feed (e.g., 'feed1', 'feed2')
webcam_anomaly_buffers = {
    "feed1": {
        "frames": deque(maxlen=SEQUENCE_LENGTH),
        "violence_probs": deque(maxlen=MIN_CONSECUTIVE_LIVE),
        "consec_count": 0,
        "last_alert_time": 0
    },
    "feed2": {
...
def pipeline_process_webcam_frame(frame, feed_id="feed1", location_name="Webcam Location", cooldown_seconds=0):
    """
    Process a single webcam frame for anomaly detection.
    Maintains a buffer per feed for temporal action classification.
    """
    global webcam_anomaly_buffers

    buffer = webcam_anomaly_buffers[feed_id]
    ...
    pred = VIOLENCE_MODEL.predict(processed, verbose=0)
    violence_prob = float(pred[0])
    buffer["violence_probs"].append(violence_prob)

    # Check for consecutive violence
    if violence_prob > VIOLENCE_THRESHOLD:
        buffer["consec_count"] += 1
    else:
        buffer["consec_count"] = 0

    # If enough consecutive violence frames and cooldown passed
    current_time = time.time()
    if (buffer["consec_count"] >= MIN_CONSECUTIVE_LIVE and
        len(buffer["frames"]) >= SEQUENCE_LENGTH and
        (current_time - buffer["last_alert_time"] > cooldown_seconds)):

        # Stage 2: Action classification
        seq = list(buffer["frames"])[-SEQUENCE_LENGTH:]
        pred = predict_action_sequence(ACTION_MODEL, seq, val_transform, device)
        action_label = IDX_TO_ACTION[pred]

        # Log to DB and alert
        _, image_buffer = cv2.imencode('.jpg', seq[0])
        image_data = image_buffer.tobytes()
        log_detection_to_db("Webcam", "anamoly", 1, image_data, action_label)
        timestamp = datetime.now()
        action = f"{action_label} Detected!! Take action"
        log_alert(location_name, "Suspicious", action, timestamp, "Webcam", 1)

        buffer["last_alert_time"] = current_time
        buffer["consec_count"] = 0  # Reset to avoid duplicate alerts

        return "anamoly", 1  # Detection made

    return "anamoly", 0  # No detection
...
def upload_file():
    ...
    print("crowd_detection_threshold: ",detection_threshold)
    print("mask_mode: ",mask_mode)
    print("cooldown_seconds: ",cooldown_seconds)
    ...
    with ThreadPoolExecutor() as executor:
        def process_video(file, input_path, selected_models):
            ...
            if 'mask' in selected_models:
                tasks.append(executor.submit(MASK_process_video_for_detections, input_path, MASK_MODEL, "Main Hall",mask_mode,cooldown_seconds,))
            ...
            if 'anamoly' in selected_models:
                ...
                tasks.append(executor.submit(
                    ...
                    input_path, output_path_anamoly, location_name, cooldown_seconds
                ))
        if file1:
...
def webcam_feed():
    ...
    if action == 'stop':
        ...
        return jsonify({'message': f'Live {feed_id} stopped.'})
    ...
    if action == 'start':
        ...
        try:
            ...
            def process_frame(frame):
                ...
                with ThreadPoolExecutor() as executor:
                    ...
                    if 'anamoly' in selected_models:
                        tasks.append(executor.submit(
                            pipeline_process_webcam_frame,
                            frame, feed_id, location_name, cooldown_seconds
                        ))
        ...
        return jsonify({'message': f'Live {feed_id} processing completed', 'results': responses})
...
def get_detection_distribution():
    ...
    models = {
        ...
        "Suspicious Activity": "anomaly_Detection"
    }
    ...
    for label, table in models.items():
        try:
            ...
            data.append({"name": label, "value": 0})  
...
def get_suspicious_activity_types():
    ...
    for activity in activity_types:
        try:
            cursor.execute(
                "SELECT COUNT(*) FROM anomaly_Detection WHERE Activity_Type = ?",
                (activity,)
            )
            count = cursor.fetchone()[0]
            data.append({"type": activity, "count": count})
        except Exception as e:
            data.append({"type": activity, "count": 0})
